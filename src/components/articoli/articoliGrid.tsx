  ...
  return (
    <div className="flex flex-col h-full">
      {articoli.map((articolo, index) => {
        const { title, subtitle, imageUrl } = articoliUnpacker(articolo)

        if (index === 0) {
          return (
            <div key={articolo.id} className="mb-4 flex-shrink-0">
              <BigCard title={title} subtitle={subtitle} imageUrl={imageUrl} />
            </div>
          )
        } else {
            ...
            return (
              <div key={articolo.id} className="flex mb-4">
                <div className="w-1/2 pr-2">
                  <MediumCard title={title} subtitle={subtitle} imageUrl={imageUrl} />
                </div>
                <div className="w-1/2 flex flex-col pl-2">
                  {articoli[index + 1] ? (
                    ...
                    <div className="flex-1 mb-2" /> // Spazio vuoto se non c'è una seconda SmallCard
                  )}
                  {articoli[index + 2] ? (
                    ...
                    <div className="flex-1 mt-2" /> // Spazio vuoto se non c'è una terza SmallCard
                  )}
                </div>
              </div>
            )
        ...
        return null // Per gli indici che sono già stati gestiti come SmallCard
      })}
    </div>
  )
}
